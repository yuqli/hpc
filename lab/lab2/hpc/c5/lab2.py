...
def precision(out, labels):
    ...
    true = labels.nonzero().cpu().numpy().tolist()  # all the nonzero values. list of pairs, 0th is the row id and 1th is col id

    rows = np.arange(250).tolist()  # list of row ids

    top1_ids = torch.topk(out, 1, dim=1)[1].squeeze().cpu().numpy().tolist()
    ...
    top3_ids = torch.topk(out, 3, dim=1)[1].squeeze().cpu().numpy().tolist()
...
def main(argv):
    ...
    try:
        ...
        fname = "result{}_{}.txt".format(optimizer_name, num_workers)
        f = open(fname, 'a')
        f.write("Optimizer {}\n".format(optimizer))
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images, labels) in enumerate(train_loader):
                ...
                if (i + 1) % 100 == 0:
                    f.write('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Precision@1: {:.4f}, Precision@3: {:.4f}'
